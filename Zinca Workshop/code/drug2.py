...
########################################
# vars2 = pd.Series(m.addVars(ther,vtype=GRB.BINARY), index=ther)

# m.addConstr(sum(vars2)==1, 'donation')

# for x in ther:
#        m.addConstr(sum(vars*cost*(t_area==x)) <= t_bud[x]+vars2[x]*50, f'{x} budget')
...
#     m.update()
#     m.optimize()
#     result.append([v.x for v in vars2])
#     m.remove(target)
# for x in result:
#     print(x)
...
# for r in np.linspace(0,20766000, 100):
#     m.remove(target)
#     target = m.addConstr(portfolio_risk<=r, 'risk')
#     m.update()
#     m.optimize()
