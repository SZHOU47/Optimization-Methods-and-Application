...
m.setObjective(portfolio_enpv-1.645*h, GRB.MAXIMIZE)

# Additional constraints
m.addConstr(h>=0)
t=L.T.dot(vars)
m.addConstr(np.square(h)>=np.sum(np.square(t)))
...
# Find minimum risk
avail_obj = m.getObjective().getValue()
m.addConstr(portfolio_enpv-1.645*h>=avail_obj)
