...
W=set(range(0,13)) #set of weeks, total of 12 weeks
...
for i in range(24):
    for j in range(24):
        if i != j:
            m1.addConstr(sum(x[i,j,k] + x[j,i,k] for k in range(13) if k != 8)<=1,f"c2{i,j}")

#Each team would play at most six home games (i.e., on their home stadium).
for i in range(24):
     m1.addConstr(sum(x[i,j,k] for j in range(24) for k in range(13) if (j != i)&(k != 8))<=6,f"c3{i}")
...
#Each team plays equal number of games against teams at their own conference and the other conference.
for i in range(12):
    m1.addConstr(sum(x[i,j,k]+x[j,i,k] for j in range(12) for k in range(13) if (j != i)&(k != 8))==6,f"c7{i}")
for i in range(12,24):
    m1.addConstr(sum(x[i,j,k]+x[j,i,k] for j in range(12) for k in range(13) if (j != i)&(k != 8))==6,f"c7{i}")

# ADD OBJECTIVE FUNCTION HERE
obj = sum(2*x[i,j,k]*E.iloc[i,j] for i in range(24) for j in range(24) for k in range(13))
...
for i in range(24):
    print(f'\nTotal distance travelled by {Team_list[i]}: {sum(2*x[j,i,k].X*E.iloc[j,i] for j in range(24) for k in range(13))} miles')
